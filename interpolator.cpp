 ...
 */
template<class T>
static std::pair<T, T> GetControlPoint(const T& p0, const T& p1, const T& p2, T (*lerp)(double, const T&, const T&)){
    auto anBar = lerp(0.5, lerp(2.0, p0, p1), p2);
    return {lerp(1.0/3, p1, anBar), lerp(-1.0/3, p1, anBar)};
}
...
static vector Lerp(double t, const vector& start, const vector& end){
...
void Interpolator::BezierInterpolationEuler(Motion * pInputMotion, Motion * pOutputMotion, int N)
{
  // students should implement this
    ...
    vector posAn0 = Lerp(1.0/3, p0->root_pos, Lerp(2.0, p2->root_pos, p1->root_pos)),
        posAn1, posBn1;
    ...
    for(int i = 0; i<MAX_BONES_IN_ASF_FILE; i++){
        boneAn0[i] = Lerp(1.0/3, p0->bone_rotation[i], Lerp(2.0, p2->bone_rotation[i], p1->bone_rotation[i]));
    }
    ...
    while(keyframe[1] < inputLength){
        ...
        if(keyframe[2] < inputLength){
            ...
            posBn1 = Lerp(1.0/3, p1->root_pos, Lerp(2.0, pm1->root_pos, p0->root_pos));
            for(int i = 0; i<MAX_BONES_IN_ASF_FILE; i++){
                boneBn1[i] = Lerp(1.0/3, p1->bone_rotation[i], Lerp(2.0, pm1->bone_rotation[i], p0->bone_rotation[i]));
            }
        }
...
void Interpolator::BezierInterpolationQuaternion(Motion * pInputMotion, Motion * pOutputMotion, int N)
{
  // students should implement this
    ...
    Quaternion<double>
        boneAn0[MAX_BONES_IN_ASF_FILE],
        boneAn1[MAX_BONES_IN_ASF_FILE],
        boneBn1[MAX_BONES_IN_ASF_FILE];
    for(int i = 0; i<MAX_BONES_IN_ASF_FILE; i++){
        Quaternion<double> q0, q1, q2;
        Euler2Quaternion(p0->bone_rotation[i].p, q0);
        Euler2Quaternion(p1->bone_rotation[i].p, q1);
        Euler2Quaternion(p2->bone_rotation[i].p, q2);
    ...
    while(keyframe[1] < inputLength){
        ...
        if(keyframe[2] < inputLength){
            ...
            for(int i = 0; i<MAX_BONES_IN_ASF_FILE; i++){
                ...
                Euler2Quaternion(p1->bone_rotation[i].p, q1);
                Euler2Quaternion(p2->bone_rotation[i].p, q2);
                auto boneNext = GetControlPoint(q0, q1, q2, &(Interpolator::Slerp));
                boneAn1[i] = boneNext.first;
                boneBn1[i] = boneNext.second;
            }
        } else {
            ...
            for(int i = 0; i<MAX_BONES_IN_ASF_FILE; i++){
                ...
                Quaternion<double> q0, q1, qm1;
                Euler2Quaternion(p0->bone_rotation[i].p, q0);
                Euler2Quaternion(p1->bone_rotation[i].p, q1);
                Euler2Quaternion(pm1->bone_rotation[i].p, qm1);

                boneBn1[i] = Slerp(1.0/3, q1, Slerp(2.0, qm1, q0));
            }
        }
        ...
        for(int f = 1; f < N + 1; f++ ){
            ...
            for(int i = 0; i < MAX_BONES_IN_ASF_FILE; i++){
                ...
                auto temp = DeCasteljauQuaternion(t, q0, boneAn0[i], boneBn1[i], q1);
                Quaternion2Euler(temp, inter.bone_rotation[i].p);
            }
            pOutputMotion->SetPosture(f + keyframe[0], inter);
        }
